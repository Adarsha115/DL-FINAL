...
# Works whether called as: python src/train.py  (from project root)
#                       or: python train.py      (from inside src/)
...
# best_model.npy and config.json are saved alongside train.py inside src/
...
def build_network(hidden_sizes, activation, weight_init):
    ...
    # Output layer — always 10 classes with Softmax
...
def main():
    ...
    dataset_name = "fashion_mnist" if args.dataset == "fashion" else "mnist"
    ...
    for epoch in range(args.epochs):
        ...
        # Save best weights (inside src/ to match skeleton)
        if val_acc > best_val_acc:
            ...
            np.save(os.path.join(SRC_DIR, "best_model.npy"),
                    np.array(weights, dtype=object))

    # Save config alongside best_model.npy
